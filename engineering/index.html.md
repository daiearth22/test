...
- [**Public Issue Tracker (for GitLab CE)**](https://gitlab.com/gitlab-org/gitlab-ce/index.html.md); please use confidential issues for topics that should only be visible to team members at GitLab.
...
1. Leave a reserve of time with unscheduled work so you can accommodate the variance. According to [Parkinson's law](https://en.wikipedia.org/wiki/Parkinson%27s_law) the work expands so as to fill the time available for its completion. This means that we're not adhering to our [iteration value](https://github.com/daijapan/test/tree/master/values/#iteration) and that for the next cycle our estimates for comparable features will be larger.
...
Just like our [OKRs](/okrs/) are so ambitious that we expect to reach about 70% of the goal this is fine for shipping [planned features](https://github.com/daijapan/test/tree/master/product/#ambitious-planning) too.
...
```javascript
javascript:void((function(d){document.cookie='gitlab_canary=' + (document.cookie.indexOf('gitlab_canary=true') >= 0 ?  'false' : 'true') + ';domain=.gitlab.com;path=/;expires=' + new Date(Date.now() + 31536000000/index.html.md).toUTCString(); location.reload();})(document));
```
...
You can find someone to review your own merge requests by looking on the [team page](/team/), or on the list of [GitLab Engineering Projects](https://github.com/daijapan/test/tree/master/engineering/projects/index.html.md), both of which are fed by `data/team.yml`.
...
As with regular reviewers, maintainers can be found on the [team page](/team/), or on the list of [GitLab Engineering Projects](https://github.com/daijapan/test/tree/master/engineering/projects/index.html.md).
