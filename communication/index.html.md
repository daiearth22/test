...
For this, we use **asynchronous communication** and are as open as we can be by communicating through public issues, [chat channels](https://github.com/daijapan/test/tree/master/communication/chat/index.html.md),
...
Please see our [social media guidelines](https://github.com/daijapan/test/tree/master/marketing/social-media-guidelines/index.html.md).
...
1. Make a conscious effort to **prioritize** your work. The priority of items depends on multiple factors: Is someone waiting for the answer? What is the impact if you delay it? How many people does it affect, etc.? This is detailed in [Engineering Workflow](https://github.com/daijapan/test/tree/master/engineering/workflow/index.html.md).
1. Use the public issue trackers on GitLab.com for everything since [we work out in the open](/2015/08/03/almost-everything-we-do-is-now-open/index.html.md). Issue trackers that can be found on the relevant page in the handbook and in the projects under [the gitlab-com group](https://gitlab.com/gitlab-com/index.html.md).
1. Pick issues from the current [milestone](https://gitlab.com/groups/gitlab-org/milestones/index.html.md).
...
1. Do not close an issue until it is [**done**](https://docs.gitlab.com/ee/development/contributing/merge_request_workflow.html#definition-of-done/index.html.md).
...
guide](https://docs.gitlab.com/ee/development/contributing/merge_request_workflow.html#merge-request-guidelines/index.html.md).
...
1. Even when something is not done, share it internally so people can comment
  ...
  Progress](/2016/01/08/feature-highlight-wip/index.html.md)** merge request so it is not merged by accident.
1. If any followup actions are required on the issue after the merge request is
  merged (like reporting back to any customers or writing documentation/index.html.md), avoid
  auto closing it.
...
1. Use **asynchronous communication** when possible (issues and email instead of chat/index.html.md).
1. Issues are preferred over email, email is preferred over chat, announcements happen on the company call agenda, and [people should be able to do their work without getting interrupted by chat](https://m.signalvnoise.com/is-group-chat-making-you-sweat-744659addf7d#.21t7089jk/index.html.md).
...
1. Sometimes synchronous communication is the better option, but do not default to it. For example, a video call can clear things up quickly when you are blocked. See the [guidelines on video chats](#video-calls/index.html.md) for more detail.
1. It is very OK to ask as many questions as you have, but ask them so many people can answer them and many people see the answer (so use issues or public chat channels instead of private messages or one-on-one emails/index.html.md) and make sure you  document the answers.
1. If you mention something (a merge request, issue, commit, webpage, comment, etc./index.html.md) please include a link to it.
...
1. It is OK to bring an issue to someone's attention with a CC ("cc @user"/index.html.md), but CCs alone are not enough if specific action is needed from someone. The mentioned user may read the issue and take no further action. If you need something, please explicitly communicate your need along with @ mentioning who you need it from.
1. Avoid creating private groups for internal discussions:
    1. It's disturbing (all user in the group get notified for each message/index.html.md).
    1. It's not searchable.
    1. It's not shareable: there is no way to add people in the group (and this often leads to multiple groups creation/index.html.md).
...
1. It is perfectly fine to create a channel, even for a single customer meeting. These channels should be named "<customer>-internal" to indicate their "internal" nature (not shared with customers/index.html.md).

## Email

1. Send one email per subject as multiple items in one email will cause delays (have to respond to everything/index.html.md) or misses (forgot one of the items/index.html.md).
...
1. If you forward an email without other comments please add FYI (for your information/index.html.md), FYA (for your action/index.html.md), or FYJ (for your judgment/index.html.md). If you forward an external request with FYJ it just means the person who forwarded it will not follow up on the request and expects you to decide if you should follow up or not.
1. Email forwarding rules are specified in the shared _Email, Slack, and GitLab Groups and Aliases_ Google Doc accessible only to people in the company. If you want to be added or removed from an internal alias (for example, "sales@gitlab.com"/index.html.md), change a rule, or add a forwarding email alias, please [suggest an edit](https://support.google.com/docs/answer/6033474?hl=en/index.html.md) in the doc.
...
1. If the subject is of value to the wider community, consider commenting on an existing issue or opening a new [issue](#everything-starts-with-an-issue/index.html.md) instead.
...
1. Unless you're in an active chat, don't break up a topic into multiple messages as each one will result in a notification which can be disruptive. Use [threads](https://get.slack.help/hc/en-us/articles/115000769927-Message-threads/index.html.md) if you want to provide extra info to the question/comment you posted. Especially in channels like `#questions` are threads very valuable to keep conversations about a certain question together.
1. Because we work globally, you may receive chat mentions at any time of day. Please consider enabling [Slack's Do not disturb functionality](https://github.com/daijapan/test/tree/master/tools-and-tips/#do-not-disturb-hours/index.html.md) so you don't get interrupted, for example, in your evenings.
...
1. Learn about [common channels and channel-naming conventions](https://github.com/daijapan/test/tree/master/communication/chat/index.html.md).
...
Apps/index.html.md) account. By default, share it with the whole company using the _Anyone at GitLab can find and access_ link sharing permission
and the _Anyone within GitLab can edit_ access permission (preferred/index.html.md) or the _Anyone within GitLab can comment_ access permission.
Easily do this by creating Google Docs within a Shared Folder in Google Drive.
1. If you have content in a Google doc that is later moved to the website or handbook, [deprecate the Google doc](#how-to-deprecate-a-google-doc/index.html.md).
...
(In the past, linking a Google Doc has led to inadvertently opening the sharing settings beyond what was intended./index.html.md)
This also helps prevent spam from people outside GitLab requesting access to a doc when clicking its link.
1. If you are having trouble finding a shared Google Doc, make sure you [Search &lt;your domain&gt;](https://support.google.com/a/answer/3187967?hl=en/index.html.md) in Google Drive.
1. In our handbook, if you find yourself wondering whether it is better to provide a public link to a Google Doc vs. writing out the content on the website, use the following guideline: Is this document frequently adapted / customized? If yes, then provide a link, making sure that the document can be _commented on_ by _anyone_ with the link. For instance, this is how we share our employment [contracts](https://github.com/daijapan/test/tree/master/contracts/index.html.md). If the document is rarely customized, then provide the content directly on the site and deprecate the Google Doc.
...
1. Add a link to the merge request or commit that moved it (if applicable/index.html.md).
...
1. We use [Zoom](https://gitlab.zoom.us/index.html.md) for the call since Google Hangouts is capped at 15 people (please be sure to mute your microphone/index.html.md). If using the Zoom web app, you can go to settings and check always mute microphone when joining a meeting.
...
1. After the general announcements, attendees share their personal interests by adding their name to the schedule or by discussing in a mixer call. The schedule is as follows:
  * Monday: Any topic
  * Tuesday: [Mixer Calls](https://github.com/daijapan/test/tree/master/communication/#mixer-calls/index.html.md), where we split off into smaller groups to discuss what we have been up to. No need to add yourself to the agenda on this day.
  * Wednesday: [Breakout Calls](https://docs.google.com/spreadsheets/d/1M-6NjK8_gAqcU1BehQVkKuDa1Ua2hTGBy8TuEDXHh3g/edit?usp=sharing/index.html.md)
  * Thursday: [Mixer Calls](https://github.com/daijapan/test/tree/master/communication/#mixer-calls/index.html.md), where we split off into smaller groups to discuss what we have been up to. No need to add yourself to the agenda on this day.
  * Friday: Any topic
...
1. It is OK to talk over people or interrupt people to ask questions, cheer for them or show your compassion. This encourages more conversation and feedback in the call. Also see the interruption item in [video calls](#video-calls/index.html.md).
...
1. Even if you cannot join the call, consider reviewing the recorded call or at minimum read through the agenda and the links from there. We often use the company call to make announcements or discuss changes in processes, so make sure to catch up on the news if you have missed a company call (or more/index.html.md).
...
We split into small groups of around eight on Tuesdays and Thursday [Company Call](https://github.com/daijapan/test/tree/master/communication/#company-call/index.html.md), using Zoom's [Breakout Rooms](https://support.zoom.us/hc/en-us/articles/115005769646-Participating-in-Breakout-Rooms/index.html.md) feature. The groups are randomly assigned based on the participants in the Company Call.
...
On the 8th of each month (or the next business day/index.html.md), we have a kickoff meeting
...
Both the retrospectives and kickoffs are live streamed (and posted/index.html.md) to the [GitLab YouTube channel](https://www.youtube.com/gitlab/index.html.md).

## Random Chat and Room
{: #random-room}

1. The [#random](https://gitlab.slack.com/archives/random/index.html.md) chat channel is your go-to place to share random ideas, pictures, articles, and more. It's a great channel to check out when you need a mental break.
1. Come hang out any time in the **random room**, an open Google Hangout video chat where anyone with a GitLab email address can come and leave as they please. The link is in the description of the `#random` chat channel; consider bookmarking it. This room is open for all and should be distinct from the [Coffee Break Calls](/culture/remote-only/#coffee-break-calls/index.html.md), which are 1x1 conversations between teammates.
...
1. Please click 'Guests can modify event' so people can update the time in the calendar instead of having to reach out via other channels. You can configure this to be checked by default under [Event Settings](https://calendar.google.com/calendar/r/settings/index.html.md)./index.html.md)
1. When scheduling a meeting we value people's time and prefer the "speedy meetings" setting in our Google Calendar. This gives us meetings of, for example, 25 or 50  minutes leaving some time to write notes etc before continuing to our next call or meeting. (This setting can be found under the calendar Settings menu at "default event duration"/index.html.md)
...
1. If you want to schedule a meeting with a person not on the team please use [Calendly](handbook/tools-and-tips/#calendly/index.html.md). Use Google Calendar directly if scheduling with a GitLabber.

## Indicating Availability

Indicate your availability by updating your own calendar using Google's ["out of office"](https://www.theverge.com/2018/6/27/17510656/google-calendar-out-of-office-option/index.html.md) feature and include the dates you plan to be away in your automated response. Note that this feature will automatically decline any meeting invitations during the time frame you select.

1. Put your planned away time including holidays, vacation, travel time, and other leave in your own calendar. Please see [Communicating your time off](https://github.com/daijapan/test/tree/master/paid-time-off#communicating-your-time-off/index.html.md) for more.
...
1. We prefer [Zoom](https://gitlab.zoom.us/index.html.md).
...
1. Google Calendar also has a [Zoom plugin](https://chrome.google.com/webstore/detail/zoom-scheduler/kgjfgplpablkjnlkjmjdecgdpfankdle?hl=en-US/index.html.md) where you can easily add a Zoom link for a video call to the invite
1. For meetings that are scheduled with Zoom:
   1. Make sure to take out the Google Hangouts link to avoid confusion.
   1. If you need more privileges on Zoom (longer meeting times, more people in the meeting, etc./index.html.md), please contact People Ops as described [specifically for Zoom](https://github.com/daijapan/test/tree/master/tools-and-tips/#sts=Zoom/index.html.md).
   1. Note that if you select to record meetings to the cloud (setting within Zoom/index.html.md), they will be automatically placed in the GitLab Videos folder in Google Drive on an hourly basis. You can find these videos in Google Drive by entering in the search bar: `title:"GitLab Videos" source:domain`.
...
1. Use a headset with a microphone, [Apple Earpods](https://www.apple.com/shop/accessories/all-accessories/headphones-speakers/index.html.md) are great. Do not use computer speakers, they cause an echo. Do not use your computer microphone, it accentuates background noise. If you want to use your [Bose headphones](https://www.bose.com/en_us/products/headphones/noise_cancelling_headphones.html/index.html.md) that is fine but please ensure the microphone is active.
1. Consider using a utility to easily mute/unmute yourself, see [Shush](https://github.com/daijapan/test/tree/master/tools-and-tips/#shush/index.html.md) in the tools section.
...
![Google Calendar - make calendar available setting](/imageshttps://github.com/daijapan/test/tree/master/tools-and-tips/google-calendar-share.png/index.html.md)

If you add blocks of time spent on recurring tasks to your Google Calendar to remind yourself to do things (e.g. "Check Google Analytics"/index.html.md), consider marking yourself "Free" for those events so that coworkers know they may schedule a meeting during that time if they can't find another convenient time.
...
1. Guide users who ask for help or give a suggestion and share links. [Improving Open Development for Everyone](/2015/12/16/improving-open-development-for-everyone/index.html.md), [Types of requests](/2014/12/08/explaining-gitlab-bugs/index.html.md).
1. When facing negative comment, respond patiently and treat every user as an individual, people with the strongest opinions can turn into [the strongest supporters](/2015/05/20/gitlab-gitorious-free-software/index.html.md).

## Writing Style Guidelines

1. {: #american-english} At GitLab, we use American English as the standard written language.
1. Do not use rich text, it makes it hard to copy/paste. Use [Markdown](https://github.com/daijapan/test/tree/master/product/technical-writing/markdown-guide/index.html.md) for things stored in git. In Google Docs use "Normal text" using the style/heading/formatting dropdown and paste without formatting.
1. Don't use ALL CAPS because if [feels like shouting](http://netiquette.wikia.com/wiki/Rule_number_2_-_Do_not_use_all_caps/index.html.md).
1. We use Unix style (lf/index.html.md) line endings, not Windows style (crlf/index.html.md), please ensure `*.md text eol=lf` is set in the repository's `.gitattributes` and run `git config --global core.autocrlf input` on your client.
1. Do not create links like "here" or "click here". All links should have relevant anchor text that describes what they link to, such as: "GitLab CI source installation documentation". Using [meaningful links](https://www.futurehosting.com/blog/links-should-have-meaningful-anchor-text-heres-why/index.html.md){:rel="nofollow noindex"} is important to both search engine crawlers (SEO/index.html.md) and people with accessibility issues.
1. Always use [ISO dates](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates/index.html.md) in all writing and legal documents since other formats [lead to online confusion](http://xkcd.com/1179/index.html.md). Use `yyyy-mm-dd`, for example 2015-04-13, and never 04-13-2015, 13-04-2015, 2015/04/13, nor April 13, 2015. Even if you use a unambiguous alternative format it is still harder to search for a date, sort on a date, and for other team members to know we use the iso standard. For quarters use 2018-Q3.
...
1. For engineering (for example production postmortems/index.html.md) we use UTC as the timezone. For all other uses we use Pacific time since we are a San Francisco based company. Please refer to time as '9:00 Pacific' and not PST since that is ambiguous during daylight savings.
...
1. All people working for GitLab (the company/index.html.md) are the [GitLab team](/team/index.html.md). We also have the [Core team](/core-team/index.html.md) that consists of volunteers.
1. Please always refer to GitLab Inc. people as GitLabbers, not employees.
1. Use [inclusive and gender-neutral language](https://techwhirl.com/gender-neutral-technical-writing/index.html.md) in all writing.
1. Always write GitLab with a capitalized G and L, even when writing GitLab.com.
1. Always capitalize the names of GitLab [features](/features/index.html.md).
...
1. Our homepage is / (with the `about.` and with `https`/index.html.md).
1. Try to use the [active voice](https://writing.wisc.edu/Handbook/CCS_activevoice.html/index.html.md) whenever possible.
1. Refer to environments that are installed and run "on-premises" by the end-user as "self-managed."
1. If you use headers, properly format them (`##` in Markdown, "Heading 2" in Google docs/index.html.md); start at the second header level because header level 1 is for titles. Do not end headers with a colon.
1. Always use an [Oxford comma](https://en.wikipedia.org/wiki/Serial_comma/index.html.md) in lists of three or more terms.
1. Always use a single space between sentences rather than two.
1. Read our [Documentation Styleguide](https://docs.gitlab.com/ee/development/doc_styleguide.html/index.html.md) for more information when writing documentation.
...
1. We segment our customers/prospects into 4 segments [Strategic, Large, Mid-Market and Small Medium Business (SMB/index.html.md)](https://github.com/daijapan/test/tree/master/business-ops/#segmentation/index.html.md).

## Situation-Complication-Implication-Position-Action-Benefit (SCIPAB/index.html.md)
...
More information can be found at [SCIPAB- Six Steps To Reach Your Audience](https://dzone.com/articles/scipab-six-steps-to-reach-your-audience/index.html.md).
...
works/index.html.md) and you can drive the robot around the boardroom. It simulates being in the space without
...
* **Beamy times**: 8am until 6pm Pacific time. [Please check the current time!](https://time.is/San_Francisco/index.html.md) on workdays and during all company events, for other times please @mention Sytse in the Slack #valley channel to see if it is OK. It is on auto connect so you'll beam right in.
...
* More info about Beam can be found at [Suitable Tech](https://suitabletech.com/index.html.md).
* Please report any questions or issues you have about the beam in the Slack #peopleops channel.

## Company phone number
{: #phone-number}

If you need to provide the details of GitLab's contact information you can take the [address from the visiting page](/visiting/index.html.md) for reference; or the [mailing address](https://github.com/daijapan/test/tree/master/people-operations/#addresses/index.html.md) of the office in the Netherlands if that is more applicable.

If a phone number is required, leave this field empty by default. If that is not possible, then use
the general number (+1-415-761-1791/index.html.md), but be aware that this number simply guides to a voice message that refers the caller back to contacting us via email.
...
This is defined in [Domain-driven design](https://en.wikipedia.org/wiki/Domain-driven_design/index.html.md) as: "A language structured around the domain model and used by all team members to connect all the activities of the team with the software.".
We use it for activities in GitLab, even ones not implemented in software.
By having ubiquitous words to identify concepts we prevent confusion over what is meant, for example we refer to [parts of our organization](/team/structure/index.html.md) as a function, department, or group depending on exactly what is meant.
Make sure that domains don't overlap, for example [organization size](https://github.com/daijapan/test/tree/master/sales/#organization-size/index.html.md) and [deal size](https://github.com/daijapan/test/tree/master/sales/#deal-sizes/index.html.md) don't reuse words to prevent overlap.
If a term is ambiguous don't use it, for example our [hiring definitons](https://github.com/daijapan/test/tree/master/hiring/#definitions/index.html.md) have roles and vacancies but avoid the ambiguous word job.
Make sure that people can infer as much as possible from the word, for example our [subscription options](https://github.com/daijapan/test/tree/master/marketing/product-marketing/#tiers/index.html.md) allow you to know if someone if using self-managed or GitLab.com.
Make sure terms don't overlap without clearly defining how and why, for example see our [tier definitions](https://github.com/daijapan/test/tree/master/marketing/product-marketing/#definitions/index.html.md).
...
1. Login to YouTube and click on the Tanuki in the upper right corner. Choose "Creator Studio"  (read the secure note in 1Password called "YouTube" for instructions on how to get access/index.html.md).
...
1. If you have a conversation that might be interesting please hit "record" (unless the [meeting is being livestreamed](https://github.com/daijapan/test/tree/master/communication/#live-streaming/index.html.md) already/index.html.md).
1. Log in to the [Zoom](https://zoom.us/index.html.md) account of the meeting and go to the menu on the right and choose "My Recordings" (it can take up to 30 minutes before the recording is available to be shared/index.html.md).
1. Select the meeting and download the recording to your computer (if you can't find the recording because it was a while ago check "Trash" in the menu on the top left and "Recover" the recording/index.html.md).
1. Go to the [YouTube upload page](https://www.youtube.com/upload/index.html.md) and log in to the GitLab account (read the secure note in 1Password called "YouTube" for instructions on how to get access/index.html.md).
1. Drag and drop your recording into the window to upload it. Keep the privacy dropdown on the default 'Public' setting (unless there is confidential material/index.html.md).
...
1. Be sure to include relevant links (for example a handbook page or presentation/index.html.md) in the description, and add the video to any relevant playlists.
...
1. There is no minimum quality, so please share it on our [GitLab Youtube channel](https://www.youtube.com/channel/UCnMGQ8QHMAnVIsI3xJrihhg/index.html.md), as long as there is nothing inappropriate or confidential.
...
1. If you record an in-person conversation with your mobile phone please hold your phone in landscape (horizontal/index.html.md) mode.
...
1. are streamed [more reliably](https://peering.google.com/#/infrastructure/index.html.md).
...
1. allow links to a [specific time in the video](https://www.h3xed.com/web-and-internet/link-to-a-specific-time-in-a-youtube-video/index.html.md).
1. can have subtitles added automatically.
1. are [zero rated by some mobile providers](https://www.t-mobile.com/offer/binge-on-streaming-video.html/index.html.md)
