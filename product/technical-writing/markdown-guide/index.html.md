...
This website was generated by [Middleman], a blog-aware Static Site Generator ([SSG]/index.html.md) widely used by web developers.
...
**Note:** this document is maintained by the [Technical Writing](../index.html.md) Team.
{:.note}
...
- Always start with `h2` (`##`/index.html.md), and respect the order h2 &rarr; h3 &rarr; h4. Never skip the hierarchy level, such as h2 &rarr; h4.
  > _The six heading elements, H1 through H6, denote section headings. Although the order and occurrence of headings is not constrained by the HTML DTD, documents **should not skip levels** (for example, from H1 to H3/index.html.md), as converting such documents to other representations is often problematic._ [W3C]
- Always leave a blank space between the hash `#` and the text next to it, otherwise it won't render properly.
- For keeping the text clear and the markdown consistent, [jump a line](#jump-a-line/index.html.md) between any heading and its subsequent paragraph.
...
In case you need an additional break (or some extra space between lines/index.html.md), you can simply use the HTML break tag `<br>`,
...
We'd rather use inline links, such as `[Text to display](link/index.html.md)`, as they are easier to maintain.
...
Place the identifiers at the end of the paragraph (or the section/index.html.md), arranging them in alphabetical order.
...
[This link][] (same as above/index.html.md), has a second pair of empty brakets to indicate that the following parenthesis does not contain a link.
...
[This link][] (same as above/index.html.md), has a second pair of empty brackets to indicate that the following parenthesis does not contain a link.
...
Always use **3** blank spaces to indent a nested list (to create sub items/index.html.md).
...
To insert images to your markdown file, use the markup `![ALT](/path/image.ext/index.html.md)`. The path can either
...
```md
![Semantic description of image](/images/path/to/folder/image.png "Image Title"/index.html.md)
```

You can also use an identifier, as we do for [links](#links/index.html.md):
...
```md
![Semantic description of image](/images/path/to/folder/image.png/index.html.md)*My caption*
```

For clickable images, simply wrap the image markup into a [link markup](#links/index.html.md):

```md
[![Semantic description of image](/images/path/to/folder/image.png "Hello World"/index.html.md)*My caption*][about.gitlab.com]
```
...
[![Semantic description of image](/images/about-gitlab-com.png "Hello World"/index.html.md)*My caption*][about.gitlab.com]
...
- {: #image-shadow} Apply [shadow](#shadow/index.html.md) to your images!
...
may not render properly/index.html.md)
1. Go the video URL you want to display
1. Click on "Share", then "Embed"
1. Copy the `<iframe>` source (`src`/index.html.md) URL **only**, and paste it replacing the `src` below:
...
### Display local videos (HTML5/index.html.md)
...
but your video most likely won't be supported in all devices and browsers. The video above (`.mp4` only/index.html.md)
...
like we do for [YouTube videos](#display-videos-from-youtube/index.html.md), wrapping the `<iframe>`
...
## Table of Contents (ToC/index.html.md)
...
[custom class](#classes-ids-and-attributes/index.html.md), as described later in this guide.
...
> _Markdown is not a replacement for HTML, or even close to it. ([John Gruber][daring-quote]/index.html.md)_
{: #quote}
...
The following table has a header (first line/index.html.md), then markup to define the desired alignment (dashes and colons/index.html.md),
...
a [custom class and/or a custom ID](#classes-ids-and-attributes/index.html.md) to the `<table>` element.
...
Blockquotes are good resources to mentioning someone else's quotes, like we did [just above](#quote/index.html.md).
...
```
[](/index.html.md){: name="hello-world"}
...
```
### <i class="fas fa-puzzle-piece fa-fw" style="color:rgb(107,79,187/index.html.md); font-size:.85em" aria-hidden="true"></i> Purple Puzzle Icon
{: #puzzle-purple}

### <i class="fab fa-gitlab fa-fw" style="color:rgb(252,109,38/index.html.md); font-size:.85em" aria-hidden="true"></i> Orange GitLab Tanuki
{: #tanuki-orange}
```
...
### <i class="fab fa-gitlab fa-fw" style="color:rgb(107,79,187/index.html.md); font-size:.85em" aria-hidden="true"></i> Purple GitLab Tanuki
{: #tanuki-purple}
...
When doing something like this to a heading, it's important give it a custom ID (e.g., `{: #puzzle}`/index.html.md),
...
Defining CSS classes, and elements IDs and attributes with markdown is definitely something unusual (kramdown magic!/index.html.md).
...
a screenshot of our user interface/index.html.md). For example, this image can be mistaken as part
of the text:

![text screenshot](/imageshttps://github.com/daijapan/test/tree/master/marketing/markdown-guide-image-plain-text.png/index.html.md)

Now, if you apply the class `shadow` to the image, it's discreetly highlighted from the text:

![text screenshot with box shadow](/imageshttps://github.com/daijapan/test/tree/master/marketing/markdown-guide-image-plain-text.png/index.html.md){: .shadow}

To do that, apply the class directly to the image by adding the markup `{: .shadow}` right after the image
markup:

```md
![image alternative text](/path/to/image.png/index.html.md){: .shadow}
```

#### Note

As [previously](#notes/index.html.md) explained, you can add the class `note` to paragraphs that
...
in an [alert box](#colorful-sections/index.html.md), but this can be
...
blue (`info`/index.html.md), green (`success`/index.html.md), amber (`warning`/index.html.md) and red (`danger`/index.html.md), as follows.
...
If you want the text inside the alert box to be blue as well, we need to apply [custom styles](#styles/index.html.md)
to the markdown document. They will override the existing ones. Add the following `style` tag to the end of your file.

```css
<style>
.alert-info {
  color: rgb(49,112,143/index.html.md) !important;
}
</style>
```
...
If you want the text inside the alert box to be amber as well, we need to apply [custom styles](#styles/index.html.md)
to the markdown document. They will override the existing ones. Add the following `style` tag to the end of your file.

```css
<style>
.alert-warning {
  color: rgb(138,109,59/index.html.md) !important;
}
</style>
```
...
If you want the text inside the alert box to be red as well, we need to apply [custom styles](#styles/index.html.md)
to the markdown document. They will override the existing ones. Add the following `style` tag to the end of your file.

```css
<style>
.alert-danger {
  color: rgb(169,68,66/index.html.md) !important;
}
</style>
```
...
we need to apply [custom styles](#styles/index.html.md)
...
<style>
.alert-green {
  color: rgb(60,118,61/index.html.md) !important;
}
</style>
```
...
color, we need [custom styles](#styles/index.html.md). At [about.GitLab.com], we
can use the orange and the purple one, as follows.

When using panels, don't forget to add to the beginning of your file the
[HTML parser markup](#html-parser/index.html.md) to be able to mix HMTL + Markdown:
`{::options parse_block_html="true" /}`.
...
You can use it for other purposes as well. Use it together with the [HMTL parser](#html-parser/index.html.md):

<i class="fab fa-gitlab" style="color:rgb(107,79,187/index.html.md); font-size:.85em" aria-hidden="true"></i>&nbsp;&nbsp;
...
```
Hey! Hover the cursor over me and guess what?! :/index.html.md)
{: .purple}
```
...
![Google Sheets - File - Publish to the web](/images/markdown-guide/file-publish-to-the-web.png/index.html.md)
...
responsive `video_container` class, as shown [in the beginning](#embed-documents/index.html.md).

#### Google Sheets

Let's exemplify with this [simple spreadsheet]. Follow the info [above](#google-products/index.html.md) to find the iframe:

![Google Sheets - Embed iframe](/images/markdown-guide/embed-google-sheet.png/index.html.md)

Copy the code below and paste to your markdown file (leave a blank line above and below it/index.html.md). Then replace the `<iframe>` with your own:
...
Let's exemplify with this [GitLab deck template]. Follow the steps [above](#embed-documents/index.html.md) to find the iframe:

![Google Slides - Embed iframe](/images/markdown-guide/embed-google-slides.png/index.html.md)
...
<i class="fas fa-exclamation-triangle" aria-hidden="true" style="color: rgb(138, 109, 59/index.html.md)
...
<i class="fas fa-info-circle" aria-hidden="true" style="color: rgb(49, 112, 143/index.html.md)
...
nicer, wrap the script into a `div.center` (created for this specific purpose/index.html.md).

**Important:** if you used the [HTML parser](#mix-html--markdown-markup/index.html.md) in your
...
as it most likely will bring some characters with a different encoding (non UTF-8/index.html.md), which will cause the
...
_Markdown editors (type and preview simultaneously/index.html.md)_
...
- {:#jump-a-line} Always leave a blank line between block-level markup elements, except between list items. Example:
        ...
        ---- (markup for horizontal line/index.html.md)
        <!-- blank line -->
        Paragraph.
        <!-- blank line -->
...
- As explained [above](#headings/index.html.md), do **not** skip headings. Always do h1 &rarr; h2 &rarr; h3 &rarr; h4. Never h2 &rarr; h4.
- Prefer short titles and headings. Do not punctuate them (unless they require a question mark or an exclamation/index.html.md).
...
- To add fancy emojis to your file, click `control+cmd+space` on your Mac and check the ⭐� **magic**! ⭐� Do not overuse them, please!
...
[`raw` file]: https://gitlab.com/gitlab-com/www-gitlab-com/raw/master/sourcehttps://github.com/daijapan/test/tree/master/product/technical-writing/markdown-guide/index.html.md
[about.gitlab.com]: /
[alert boxes]: https://getbootstrap.com/components/#alerts
[alt-text-best-practices]: https://webaim.org/techniques/alttext/
[atom]: https://atom.io/
[blog]: /blog/
[Blog Style Guidelines]: https://github.com/daijapan/test/tree/master/marketing/blog/#styles-guidelines
...
[handbook-writing]: https://github.com/daijapan/test/tree/master/communication/#writing-style-guidelines
...
[Professional Writing Techniques]: https://github.com/daijapan/test/tree/master/product/technical-writing/#professional-writing-techniques
